...
const Product = () => {
  ...
  useEffect(() => {
    const fetchProductData = async () => {
      try {
        ...
        setMainImage(productResponse.data.images?.[0]?.url || '/assets/placeholder.jpg');
  ...
  useEffect(() => {
    if (product) {
      setSelectedColor(product.colors?.[0]?.name || "blue");
      setSelectedSize(product.sizes?.[0]?.name || "M");
      setQuantity(1);
      setWishlist(false);
      setPincode("");
      setPincodeStatus(null);
      setMainImage(product.images?.[0]?.url || '/assets/placeholder.jpg');
    }
  }, [product]);
  // Dummy reviews
  const reviews = [
...
console.log("Product state:", product);
              ...
              <button
                onClick={async () => {
                  const result = await addToCart({
                    ...
                    color: { name: selectedColor, code: selectedColor === 'blue' ? '#0000FF' : '#FFFFFF' },
                    size: selectedSize,
                    quantity,
                  });
                ...
                className="w-full bg-gradient-to-r from-blue-900 to-blue-600 text-white py-3 rounded-full font-bold shadow-lg hover:from-blue-600 hover:to-blue-500 transition disabled:opacity-50"
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
    {relatedProducts.map((rel) => (
      ...
      >
        <img
          src={rel.images?.[0]?.url || "/assets/placeholder.jpg"}
          alt={rel.name}
        ...
        {/* ✅ Add proper rating rendering */}
        <div className="flex items-center gap-1 text-yellow-400 text-sm mt-1">
          {Array.from({ length: 5 }).map((_, i) => (
            <span
              key={i}
              className={
                i < Math.floor(rel.rating?.average || 0)
                  ? "text-yellow-400"
                  : "text-gray-300"
              }
            >
          ...
          <span className="text-xs text-gray-500 ml-1">
            ({rel.rating?.average || 0} / 5, {rel.rating?.count || 0} reviews)
          </span>
        </div>
      </Link>
    ))}
