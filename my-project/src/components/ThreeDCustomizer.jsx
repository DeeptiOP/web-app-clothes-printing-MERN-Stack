...
function ThreeDCustomizer() {
  ...
  const handleAddToCart = async () => {
    ...
    const customProduct = {
      id: `custom-${Date.now()}`,
      name: `Custom T-Shirt - ${customText || 'Design'}`,
      price: 25, // Base price for custom t-shirt
      ...
      customization: {
        ...
        textFont: 'Arial',
        textPosition: 'center',
        uploadedImage: uploadedImage[printSide],
        selectedDesign: null,
        additionalNotes: `Custom design on ${printSide} side`
      }
    };

    const result = await addToCart(customProduct);
    
    if (result.success) {
      // Store design data for cart page
      localStorage.setItem("cartDesign", JSON.stringify(designData));
      navigate("/cart");
    } else {
      alert(result.message || 'Failed to add custom design to cart');
    }
  };
